...
Ts=1
q=[]
#连接并开启陀螺仪（需关闭主程序自稳，否则冲突）
try:
  i2cc = I2C(scl=Pin(32), sda=Pin(33))#直插接口
  #i2cc = I2C(scl=Pin(32), sda=Pin(33))集成接口
...
def cal_adjust(CG_Y,l,xk,sita,period):   #计算重心偏移量函数,使得机器人重心投影始终落在三角形面
  result=(CG_Y+period*(l+xk)/4+100*tan(sita))  #计算高度暂时固定在典型的100
  #print(result)   #打印出调整量，用于调试
  return result
  

def cal_w(CG_X,CG_Y,l,xf,h,speed,t):   #WALK步态主计算函数，相序 1-2-3-4
    global d,z
